(() => {
  ...
  function evalResolvedPick(pickText, gh, ga) {
    ...
    // Normalize + as combo separator (e.g. "X2+OVER 1.5"), but only when between
    // non-digit chars to avoid breaking decimals like "1.5"
...
VALUE_PICKS: "Quote di Valore",
  ...
  async function loadHistoryMonth(year, month0) {
    ...
    // Find all dates that have picks without a Supabase result (WIN/LOSE/VOID).
    // We'll fetch live fixture data for those dates to resolve them on the fly.
    // Cap to last 14 days and max 10 API calls to stay within quota.
    const now = new Date();
    const cutoffDate = new Date(now);
    cutoffDate.setUTCDate(cutoffDate.getUTCDate() - 14);
    const cutoffStr = dateToYMD_UTC(cutoffDate);

    const datesNeedingLive = new Set();
    for (const p of picks) {
      if (isExcludedCategory(p.category)) continue;
      const pd = String(p.match_date || "").trim();
      if (!pd || pd < cutoffStr) continue;
      const fid = String(p.fixture_id || "").trim();
      const pickText = String(p.pick || "").trim();
      const key = pd + "__" + fid + "__" + pickText;
      const res = resultMap.get(key);
      const currentResult = String(res?.result || "").toUpperCase();
      if (currentResult !== "WIN" && currentResult !== "LOSE" && currentResult !== "VOID") {
        datesNeedingLive.add(pd);
      }
    }

    // Fetch live/historical fixtures for all qualifying dates (newest first, max 10)
    const liveFixtureMaps = new Map(); // dateStr -> Map<fixtureId, fixture>
    const datesToFetch = Array.from(datesNeedingLive).sort().reverse().slice(0, 10);
    for (const d of datesToFetch) {
      liveFixtureMaps.set(d, await fetchLiveFixtureMap(d));
    }
    ...
    function pickResult(p) {
      ...
      const key = pd + "__" + fid + "__" + pickText;
      const res = resultMap.get(key) || {};
      const dateMap = liveFixtureMaps.get(pd);
      const liveFixture = dateMap ? dateMap.get(fid) : null;
