(() => {
  ...
  async function loadHistoryMonth(year, month0) {
    ...
    const todayLocalStr = dateToYMD_Local(new Date());
    let html = `<div class="day-accordion-list">`;
    ...
    for (const day of sortedDays) {
      ...
      const winrateDay = played > 0 ? ((s.wins / played) * 100).toFixed(1) : "-";
      const roiClass = roiDay === "-" ? "neutral" : (parseFloat(roiDay) >= 0 ? "pos" : "neg");
      const isToday = day === todayLocalStr;

      html += `
        <details class="day-accordion ${isToday ? "open-today" : ""}" ${isToday ? "open" : ""}>
          <summary class="day-summary-row">
            <div class="day-summary-main">
              <div class="day-summary-date">
                <strong>${formatDateIT(day)}</strong>
                ${isToday ? '<span class="day-badge">Oggi</span>' : ""}
              </div>
              <div class="day-summary-sub">${played} picks giocate · ${s.wins} win · ${s.loses} lose</div>
            </div>
            <div class="day-summary-stats">
              <span class="day-stat-pill ${roiClass}">${roiDay === "-" ? "-" : `ROI ${roiDay}%`}</span>
              <span class="day-stat-pill ${s.profit >= 0 ? "pos" : "neg"}">${profitStr} u</span>
          ...
          <div class="day-accordion-body">
            <div class="day-detail-grid">
              <div class="day-detail-card">
                <span>Picks giocate</span>
                <strong>${played}</strong>
              </div>
              <div class="day-detail-card pos">
                <span>Win</span>
                <strong>${s.wins}</strong>
              </div>
              <div class="day-detail-card neg">
                <span>Lose</span>
                <strong>${s.loses}</strong>
              </div>
              <div class="day-detail-card ${roiClass}">
                <span>ROI giornaliero</span>
                <strong>${roiDay === "-" ? "-" : roiDay + "%"}</strong>
              </div>
              <div class="day-detail-card ${s.profit >= 0 ? "pos" : "neg"}">
                <span>Profitto</span>
                <strong>${profitStr} u</strong>
              </div>
            </div>
            <div class="day-detail-note">
              Winrate giornata: <strong>${winrateDay === "-" ? "-" : winrateDay + "%"}</strong>.
              Una sola pick valida per fixture, stake fisso 1.
            </div>
          </div>
        </details>
      `;
