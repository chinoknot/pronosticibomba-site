...
def normalize_match(raw: dict) -> dict:
    fixture_id = raw.get("fixture_id") or nested_get(raw, "fixture.id") or raw.get("id")
    league = raw.get("league_name") or nested_get(raw, "league.name") or raw.get("league") or ""
    country = raw.get("country") or nested_get(raw, "league.country") or ""
    home = raw.get("home_team") or nested_get(raw, "teams.home.name") or nested_get(raw, "home.name") or ""
    away = raw.get("away_team") or nested_get(raw, "teams.away.name") or nested_get(raw, "away.name") or ""
    date = raw.get("date") or nested_get(raw, "fixture.date") or ""
    time = raw.get("time") or parse_dt(nested_get(raw, "fixture.date")) or parse_dt(raw.get("kickoff"))
...
def write_xlsx(path: Path, selected: List[dict], all_candidates: List[dict], summary: dict) -> None:
    if Workbook is None:
        return
    wb = Workbook()
    ws = wb.active
    ...
    ws2 = wb.create_sheet("all_candidates")
    if all_candidates:
        headers2 = list(all_candidates[0].keys())
        ws2.append(headers2)
    ...
    for k, v in summary.items():
        if isinstance(v, (dict, list)):
            ws3.append([k, json.dumps(v, ensure_ascii=False)])
        else:
            ws3.append([k, v])
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default="_match_center_filter_dry_run_v2")
    ...
    summary = {
        ...
        "breakdown": {rule["label"]: counts.get(rule["label"], 0) for rule in MARKET_RULES},
    }

    write_json(base.with_name(base.name + "_selected.json"), selected)
    write_csv(base.with_name(base.name + "_selected.csv"), selected)
    write_json(base.with_name(base.name + "_all_candidates.json"), all_candidates)
    write_csv(base.with_name(base.name + "_all_candidates.csv"), all_candidates)
    write_json(base.with_name(base.name + "_summary.json"), summary)
    write_json(base.with_name(base.name + "_debug_samples.json"), debug_samples)
    write_report(base.with_name(base.name + "_report.md"), target_date, cache_file, len(normalized), all_candidates, selected)
    write_xlsx(base.with_name(base.name + ".xlsx"), selected, all_candidates, summary)

    print("=" * 72)
    print("MATCH CENTER DRY RUN V2")
    ...
    print("\n# Breakdown after cap:")
    for rule in MARKET_RULES:
        print(f"- {rule['label']}: {counts.get(rule['label'], 0)}")
    print(f"\n# Output dir: {out_dir}")
    return 0
